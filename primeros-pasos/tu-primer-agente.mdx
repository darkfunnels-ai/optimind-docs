...
> Estamos armando tu agente.

La conversación gasta saldo, como todo lo que hace la IA: para eso tu cuenta nace
con **US\$ 5** de anticipo (lo cuenta [Planes y precios](/primeros-pasos/planes-y-precios)).
...
<Steps>
  ...
  <Step title="Confirma la creación del agente">
    En cuanto sabe qué vendes y cómo, te pide permiso para crear el agente:
    aparece una tarjeta **«Confirmar: Creando el embudo»** con el **nombre** y el
    **objetivo** propuestos, y los botones **Aprobar** y **Rechazar**.
    ...
    es dinero tuyo. Nace activo, con el manual vacío, con su **persona** ya
    activa (el nombre y la forma de hablar con que atiende; la afinas después
    pulsando el chip de la persona en el manual) y sin número de WhatsApp:
    existe, y todavía no vende nada.
  </Step>
  <Step title="Arma el manual capítulo por capítulo">
    El panel te lleva al **Manual** de tu agente y el copiloto te lo enseña
    señalando la pantalla: qué es un capítulo, sus PASOS, la línea que pasa al
    siguiente. Desde ahí trabajan **una etapa por vez**: te pregunta por el
    saludo, resume lo que le contaste y propone **el capítulo 1** —verás
    **«Capítulo 1 de N»** con el anillo de progreso— mientras a la derecha una
    prueba automática lo recorre con un cliente de ejemplo. Si algo no te gusta,
    se lo dices y lo vuelve a proponer.

    Con **Aprobar** se escribe **solo ese capítulo**, y siguen con la etapa
    siguiente. Después del capítulo 1 te pide **los archivos de tu producto**
    (catálogo, flyer, lista de precios) y con ellos escribe la **sección
    compartida** del manual, que también apruebas tú. El cierre es el QR de
    WhatsApp.
  </Step>
</Steps>
...
un capítulo; la propuesta espera en el hilo con **Aprobar** / **Rechazar**, y
hasta que pulses Aprobar tu manual sigue exactamente como estaba.
</Warning>
...
**Armar tu agente así gasta centavos de tu anticipo**, y por eso es el camino
normal antes de conectar WhatsApp: tu cuenta nace con **US\$ 5** justo para esto.

<Note>
**No es un regalo, es un adelanto.** Esos US\$ 5 son parte de tus **US\$ 10** de
bienvenida: el día que conectas WhatsApp se acredita el resto, así que verás
llegar US\$ 5 (más US\$ 25 si vienes referido). Si el anticipo se agotara antes de
conectar, el copiloto te lo dice —*«Tu cuenta no tiene saldo disponible»*— y la
salida es conectar tu WhatsApp o recargar.
</Note>
...
Conversar es la única vía asistida para el embudo entero, y es a propósito: es la que comprueba que el
guion tenga sus pasos numerados, su bloque de contexto compartido y su
personalidad antes de proponerte nada. Hubo un botón que escribía el embudo de
una sola pasada, sin esas comprobaciones, y se retiró: salían guiones peores.

## Si el copiloto dice que no tienes saldo

El alta corre contra el mismo saldo que todo lo demás. Si ves *«Tu cuenta no
tiene saldo disponible. Recarga créditos para seguir usando el copiloto»*, el
anticipo se agotó: **conecta tu WhatsApp** (llega el resto de tus créditos de
bienvenida) o **recarga**. Si además tu prueba venció, hace falta un plan.

**El alta no se pierde**: el agente y los capítulos que ya aprobaste siguen ahí,
y también puedes seguir [en el manual](/guias/manual), capítulo por capítulo, con
versiones y sin gastar nada.

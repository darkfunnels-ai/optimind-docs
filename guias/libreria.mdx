...
vacío. La condición de envío de ese vínculo se cambia con `set_file_trigger`;
el mensaje que acompaña al archivo sí sigue siendo cosa del panel.
...
- **La condición de envío** (`trigger_condition`): es el nombre con el que el
  manual llama a ese archivo. **El estándar —y el default si no la escribes—
  es el NOMBRE del archivo sin extensión**: archivo «lista_precios.pdf» ⇒
  condición «lista_precios». Sigue leyendo, porque es la pieza central.
...
1. **Mantén condición = nombre y escribe el marcador con ese nombre.** Ojo:
   `save_manual_chapters` —la vía de edición suelta del puente— guarda la frase
   que le des, exista el archivo o no; el que rechaza el manual entero y te
   dice qué marcador falla es `apply_funnel` (y el asistente de creación del
   panel), que valida contra el nombre del archivo y contra su condición viva.
   Desde el puente, antes de escribir un marcador, lista los archivos.
2. **Dale una condición distinta a cada archivo.** Si dos archivos enviables
   del mismo agente comparten frase, la frase deja de identificar a uno solo:
   al entregar, el agente declara la ambigüedad y no manda nada hasta que se le
   pida por el nombre exacto del fichero. Ojo: la Librería deduplica por
   CONTENIDO, no por nombre — dos archivos distintos pueden llamarse casi igual
   («catalogo.pdf» y «catalogo.jpg»), y con condición = nombre heredarían la
   misma condición. Renombra uno de los dos antes de vincular ambos como
   enviables al mismo agente.
3. **Renombrar arrastra los marcadores — con un freno deliberado.** Cambiar la
   condición de un vínculo (desde el panel o con `set_file_trigger` del puente)
   reapunta los marcadores ya escritos, y renombrar el ARCHIVO, cuando su
   condición era el nombre (el estándar), arrastra también la condición. El
   freno: si OTRO vínculo vivo del agente conserva la condición vieja (el caso
   de los homónimos de la regla 2), no se toca nada — el marcador seguiría
   nombrando un archivo real y reapuntarlo se lo robaría. La respuesta dice
   cuántos marcadores se movieron (`manual_retagged`); un `0` tras un rename es
   la señal de revisar. Si en cambio quitas el vínculo y lo vuelves a crear con
   otra frase, los marcadores **no** se reapuntan y quedan huérfanos.
...
- **De un vínculo ya creado solo edita la condición de envío.** Eso se hace con
  `set_file_trigger`, que arrastra los marcadores que la nombraban — con el
  mismo freno de la regla 3 de arriba (`manual_retagged` dice cuántos). El
  mensaje que acompaña al archivo (`message`) se cambia en el panel.

...
**Si lo armas conversando, el copiloto te pide los archivos.** Después del
primer capítulo aparece en el panel una tarjeta para soltar todo el material de
tu producto (catálogo, flyer, lista de precios, fichas, fotos). Suben a la
Librería con tu cuenta, el copiloto los lee y te propone dos cosas que apruebas
tú: qué archivo **envía** el agente por WhatsApp y cuál solo **consulta** para
responder, y la **sección compartida §** —los datos verificables, las respuestas
a las dudas frecuentes y las reglas del negocio— escrita con lo que dicen esos
archivos. Lo que el archivo no diga, no se afirma.
...
debajo del nombre. Es la parte que más vas a tocar. Para añadir una línea al
final, pasa el ratón por el capítulo: aparece una línea fina **«＋ añadir
instrucción»**; al hacer clic se convierte en la caja de escribir (en un
capítulo vacío y en el teléfono se ve siempre).
...
**Los botones de la cabecera.** Para no llenar la fila, solo el chip de
ejecución está siempre a la vista. Al pasar el ratón por el capítulo aparecen
dos botones cortos, y el menú **⋯** guarda el resto:

- **«Ver»** (en el menú, «Ver como lo recibe el agente») — abre una ventana con
  el texto de ese capítulo tal como queda guardado.
- **«✦ Reescribir»** — selecciona el capítulo entero y abre las acciones de
  ✦ IA.
- En el menú **⋯**: **«Probar desde aquí»** (arranca el chat de prueba en ese
  capítulo) y **«Eliminar capítulo»** (mira la sección de más abajo antes de
  usarlo).

En el teléfono no hay «pasar el ratón»: los botones se ven siempre.
...
A la izquierda, cada capítulo lleva un **punto de color**: rojo si el Doctor
le encontró un error, ámbar si tiene avisos, verde si no hay nada. Pasa el
ratón por la fila y el tooltip te dice qué le pasa; un clic en el punto te
lleva a la línea del primer hallazgo. Bajo el rótulo «Manual» se resume el
total («1 error · 2 avisos»). Además, **«1×»** marca los capítulos que se
ejecutan una sola vez y **«vacío»** los que no tienen líneas propias (solo les
aplican las secciones §).

### Plegar capítulos

Con seis capítulos el documento se hace largo. El chevrón a la izquierda de
cada rótulo **pliega** el capítulo: queda la cabecera y una línea que lo
resume (cuántos PASOS tiene, qué archivos y herramientas menciona, a qué
capítulos sale y cuántas secciones § le aplican). Un clic en el chevrón o en
ese resumen lo vuelve a abrir. En el índice, **«Plegar todo»** pliega todos
los capítulos de una vez y **«Desplegar todo»** los abre.

Plegar no toca el texto: lo escrito sigue ahí, solo deja de verse. Y cualquier
salto (un capítulo del índice, «Ir al sitio» del Doctor, el primer uso de un
recurso, el capítulo que recorre el chat de prueba) despliega el capítulo
antes de llevarte.
...
- **El subrayado rojo del editor compara contra los apodos**, no contra los
  ...
  acaba la duda.
- **Cada mención de archivo lleva un punto de estado** en el editor: verde si
  el archivo está activo, ámbar si está **pausado** en la Librería (existe,
  pero el agente no lo enviará aunque el manual lo pida) y rojo si no existe.
  Pasa el ratón por la mención y sale una tarjeta con el archivo (tipo y
  tamaño), su estado, la condición que lo dispara y en qué otros capítulos se
  usa; desde ahí puedes **ver el archivo**, **cambiarlo por otro**,
  **reactivarlo** si está pausado o, si no existe, **reconectarlo** a otro o
  abrir Recursos para subirlo.
...
Hoy hace **dieciséis comprobaciones**. Entre ellas:

- **PASOS fuera de orden, referencias a un paso que no existe y capítulos
  intermedios que no avanzan a ninguno** — la misma regla con la que el
  asistente rechaza un embudo nuevo, corriendo mientras escribes; un salto de
  numeración (PASO 1, 2, 3, 5) se renumera a un clic, referencias incluidas,
...
El botón **Probar** de la barra abre, junto al documento, un chat de prueba con
tu agente. Escribes como si fueras el cliente y el agente responde con el
manual **guardado**: es el mismo motor que atiende WhatsApp, pero nada sale por
WhatsApp.

Mientras el agente contesta, el manual se mueve contigo: el capítulo en el que
está se ilumina, pulsa mientras piensa, y el PASO cuya herramienta se está
ejecutando respira. Debajo de cada respuesta ves la traza: en qué capítulo
entró y qué herramientas usó. Un clic en el capítulo de la traza te lleva a él.

- **Probar desde aquí**, en el menú ⋯ de la cabecera de un capítulo,
  reinicia el hilo de prueba en ese capítulo. Lo mismo hace **Empezar en**
  dentro del panel.
- Si tienes **cambios sin guardar**, el panel lo dice: el agente no los ve
  hasta que guardes. **Guardar y probar** guarda y sigue.

<Note>
El hilo de prueba es real: consume saldo como cualquier conversación, y lo que
el agente registre en él (una venta, un pedido) queda anotado como en cualquier
chat. Solo la entrega se queda dentro del panel.
</Note>

## Borrar un capítulo

Al elegir «Eliminar capítulo» en el menú ⋯ se abre una confirmación con el botón **«Eliminar
...
CLAVE:**—, las **herramientas** y los **archivos** del capítulo. Entre el nombre y el rol pueden colarse dos avisos del sistema: desde

...
The assistant authenticates over **OAuth with your own DarkFunnels account**, and
...
DarkFunnels team: revocation from the dashboard is not available yet.
...
Your end customers are not DarkFunnels users: they messaged your business on
...
This closes no operation. **No tool needs a customer's number to act on their
chat**: replying, switching a chat's mode and creating reminders all address the
conversation, not the number. The one dent is convenience: customer search does
accept a phone number, so your assistant can't chain one it just read masked. The one tool that does take a phone number is
...
Sales are recorded by your WhatsApp agent inside DarkFunnels; the bridge only
